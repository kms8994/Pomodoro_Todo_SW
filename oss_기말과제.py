...
from tkinter import messagebox # messagebox 모듈 추가
...
class PomodoroApp:
    def __init__(self, root):
        ...
        # mixer 초기화는 try-except 블록으로 감싸서, 만약 pygame 설치가 불완전하거나 문제가 있을 경우 앱이 완전히 죽는 것을 방지합니다.
        try:
            mixer.init()
            self.mixer_initialized = True
        except Exception as e:
            print(f"Pygame mixer 초기화 오류: {e}. 알람 기능이 작동하지 않을 수 있습니다.")
        ...
        # '모든 할 일 저장하기' 버튼 제거 (앱 종료 시 자동 저장되므로)
    ...
    def load_todos(self):
        ...
        if os.path.exists(self.todo_file):
            try:
                ...
                # JSON 파일이 손상되었거나 찾을 수 없을 때 (FileNotFoundError는 os.path.exists로 걸러지지만, 만약을 위해)
                print(f"To-Do 불러오기 오류: {e}. 새 파일로 시작합니다.")
            except Exception as e:
                print(f"예상치 못한 To-Do 불러오기 오류: {e}. 기존 데이터는 유지하고, 파일 로드 실패로 인한 문제는 무시합니다.")
    ...
    def add_todo(self, day):
        ...
        if new_todo_text:
            ...
            messagebox.showwarning("입력 오류", "할 일 내용을 입력해주세요.")
    ...
    def clear_all_todos(self):
        response = messagebox.askyesno("전체 초기화", "정말로 모든 할 일을 초기화하시겠습니까? 이 작업은 되돌릴 수 없습니다.")
        if response:
            for day in self.days:
                self.new_todo_entries[day].delete(0, tk.END)
                self.todos[day] = []
            ...
            messagebox.showinfo("초기화 완료", "모든 할 일이 초기화되었습니다.")
    ...
    def _on_closing(self):
        self.save_all_todos()
        self.stop_timer()
        if self.mixer_initialized: # mixer가 성공적으로 초기화되었을 때만 quit 호출
            mixer.quit()
        self.root.destroy()
    ...
    def play_alarm(self):
        if not self.mixer_initialized: # mixer가 초기화되지 않았다면 알람 재생 안 함
            print("Pygame mixer가 초기화되지 않아 알람을 재생할 수 없습니다.")
            return

        if not os.path.exists(self.alarm_path):
            print(f"알람 파일 '{self.alarm_path}'을(를) 찾을 수 없습니다. 알람을 재생할 수 없습니다.")
            messagebox.showerror("알람 오류", "알람 파일을 찾을 수 없습니다! 'alarm.mp3' 파일이 앱과 같은 폴더에 있는지 확인해주세요.")
        ...
        def alarm_loop():
            try:
                ...
                if self.mixer_initialized and mixer.music.get_busy(): # 초기화된 경우에만 stop 호출
                    mixer.music.stop()
                self.alarm_playing = False
                self.root.after(0, self.stop_alarm_btn.config, {'state': "disabled"})
    ...
    def stop_alarm(self):
        ...
        # alarm_thread가 종료될 시간을 주기 위해 잠시 대기
    ...
    def focus_done(self):
        ...
        def wait_then_break():
            while self.is_running and not self.stop_alarm_flag:
                time.sleep(0.2)
            if self.is_running: # 사용자가 중지 버튼을 누르지 않은 경우에만 휴식 시작
    ...
    def start_break(self):
        try:
            ...
            total_seconds = minutes * 60 + seconds
            if total_seconds <= 0:
                messagebox.showwarning("시간 설정 오류", "휴식 시간은 0보다 커야 합니다.")
                self.stop_timer()
                return
    ...
    def break_done(self):
        ...
        def wait_then_focus():
            ...
            if self.is_running: # 사용자가 중지 버튼을 누르지 않은 경우에만 다시 공부 시작
